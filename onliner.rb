  ...
  module Rack
    class Onliner
      ...
      def self.process(onliner)
        puts "Onliner.process: "
        unless onliner && onliner.user_id
          puts "Onliner.process: no user_id for onliner "          
          return
        end
        ...
        puts "Onliner.process: Added onliner for user #{onliner.user_id}"
        puts "Onliner.process: users =  #{@@users.inspect}"        
        ...
        if time_now > @@last_update_time + BUFFER_TIME
          ...
          Onliner.delete_old do
            ...
            if run_time > Onliner::FADEOUT_TIME
              puts 'Onliners processing took longer than the fadeout time!! Contact Adam or Kevin.'
            end
          end
        end
      end
      ...
      def self.delete_old(&blk)
        ...
        puts "Onliner.delete_old: SQL = #{sql.inspect}"
        df1 = ourstage_dbconn.execute(sql)

        df1.callback do
          puts "Onliner.delete_old: success deleting old onliners"

          # delete all onliners for users that are to be updated
          sql2 = %Q(DELETE FROM onliners where user_id IN (#{@@users.keys.join(',')});)
          puts "Onliner.delete_old: SQL2 = #{sql2.inspect}"
          df2 = ourstage_dbconn.execute(sql2)

          df2.callback do
            puts "Onliner.delete_old: success deleting by user_id"
            blk.call if block_given?
          end
          
          df2.errback do |ex|
            if ex.is_a?(PG::Result)
              puts "Onliner.delete_old: deleting by user_id, Exception = #{ex.error_message}"
            else
              puts "Onliner.delete_old.: deleting by user_id, Exception = #{ex.inspect}"
            end
            raise ex
          end
        end
        ...
        # error callback
        df1.errback do |ex|
          puts "Onliner.delete_old: Exception for deleting old onliners"
          if ex.is_a?(PG::Result)
            puts "Onliner.delete_old: Exception = #{ex.error_message}"
          else
            puts "Onliner.delete_old: Exception = #{ex.inspect}"
          end
          raise ex
        end
      end
      ...
      def self.insert_new
        @@users.values.each do |onliner|          
          begin
            ...
            puts "Onliner.insert_new: Exception = #{ex.to_s}, failed to create onliner record for #{onliner.inspect}"
          end
        end
      end

      def self.create!(onliner)
        sql = %Q(INSERT INTO onliners (user_id, user_level, activity, parameters, ip_addr, updated_at) VALUES (#{onliner.user_id},#{onliner.user_level}, #{onliner.activity}, #{onliner.parameters}, #{onliner.ip_addr},#{onliner.updated_at}) )
        puts "Onliner.create!: SQL = #{sql.inspect}"
